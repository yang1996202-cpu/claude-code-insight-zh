...
class SessionLoadingTest(unittest.TestCase):
    def test_merge_jsonl_meta_facet(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(session.duration_minutes, 1)
            self.assertEqual(session.meta["elapsed_duration_minutes"], 1)
            self.assertEqual(session.user_message_count, 1)
            self.assertEqual(session.meta["jsonl_user_row_count"], 1)
            self.assertEqual(session.git_pushes, 0)
            self.assertEqual(session.meta["git_pushes"], 2)
            self.assertEqual(session.meta["git_activity_count"], 0)
